...
function StatReset(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(0.6, 0.6);
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 2;
}
StatReset.prototype.type = "statreset"
...
function Water(pos) {
  ...
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 1.3;
}
...
StatReset.prototype.act = function(step) {
...
Water.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed * 0.5;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
Rubber.prototype.act = function(step) {
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  else if (type == "statreset") {
		this.actors = this.actors.filter(function(other) {
			return jumpSpeed = 17, other!= actor;
		});
	}
  else if (type == "rubber") {
		this.actors = this.actors.filter(function(other) {
			return jumpSpeed = 20, other!= actor;
		});
	}
  else if (type == "coin") {
    this.actors = this.actors.filter(function(other) {
